...
//global $wp_query;
 $j_term = get_queried_object();
$term_id = get_queried_object_id();
get_header( 'shop' ); ?>
                 <?php
				 //print_r($_GET['price']);exit;
				 if(!empty($_GET['offered_price'])){
  ...
  <ul  class="mtree transit mtree_mainwidth">
              <li class="mtree-node mtree-closed" style="opacity: 1; transform: translateY(0px);-webkit-transform: -webkit-translateY (85px);"><a href="javascript:void(0)" style="cursor: pointer;">Price</a>
                    ...
                    <?php for($start=0; $start < $max_price; $start+=$price_range){
						if(in_array($start.'-'.($start+$price_range),$_GET['price']) && !empty($_GET['price'])){
							$sel = 'checked="checked"';
						}?>
                        <li><span class="side_checkbox"> <input type="checkbox" value="<?php echo $start;?>-<?php echo $start+$price_range;?>" <?php echo $sel;?> name="price[]"></span>$<?php echo $start;?> - $ <?php echo $start+$price_range;?><!--</a>--></li>
						<?php unset($sel);} ?>
                           </ul>
             </li>
    <li class="mtree-node mtree-closed" style="opacity: 1; transform: translateY(0px);-webkit-transform: -webkit-translateY (85px);"><a href="javascript:void(0)" style="cursor: pointer;">Sale</a>
                    <ul class="brand advanced-filters  brand_filter mtree-level-1" style="overflow: hidden; height: 0px; display: none;">
                        <li><span class="side_checkbox"> <input type="checkbox" value="1" name="offered_price" <?php if(!empty($_GET['offered_price'])){ ?> checked="checked"<?php } ?>></span>Offers</li>
                           </ul>
             </li>
 ...
 <?php $all_filters = unserialize($cur_term_option);?>  
  ...
  <li <?php if($_GET[$tax->attribute_name]){?>class="<?php echo $tax->attribute_name;?> mtree-node mtree-active mtree-open"<?php } ?>><a href="javascript:void(0)"><?php echo $tax->attribute_name;?></a>
          ...
          <ul class="<?php echo $tax->attribute_name;?> advanced-filters <?php if($_GET[$tax->attribute_name]){?> filter_block <?php } ?> brand_filter">
            ...
            <li><span class="side_checkbox"> <input type="checkbox" name="<?php echo $tax->attribute_name;?>[]" value="<?php echo $term_id;?>" id="<?php echo $term_id;?>" /></span><?php echo  substr($term_name_obj->name, 0, 20); ?><!--</a>--></li>
            <?php } ?>
              </ul>
               </li>
            <?php } ?>
      ...
      <!--<div class="grid-item large--three-quarters">-->
        ...
        <div class="section-header">
          <div class="section-header--left">
              <div class="collection-view"> <?php //do_action( 'woocommerce_bredcrumbs' );?><a title="Grid view" class="collection-view--active" href="<?php echo HOME;?>">Home  &nbsp;<span> &gt;</span>&nbsp;&nbsp;</a> <?php if(isset($term_id)){?>
			  <?php $term_name_cur = get_term_by('slug',$j_term->slug, 'product_cat');
			  //echo "<pre>";print_r($term_name_cur);exit;
