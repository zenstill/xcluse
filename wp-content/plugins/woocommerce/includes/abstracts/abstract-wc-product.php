...
class WC_Product {
	...
	public function get_single_price_html( $price = '' ) {
		...
		if ( $this->get_price() > 0 ) {
			...
			if ( $this->is_on_sale() && $this->get_regular_price() ) {
				...
				$price .= $this->get_price_html_from_to_j( $display_regular_price, $display_price ) . $this->get_price_suffix();
			...
			if ( $this->is_on_sale() && $this->get_regular_price() ) {
				...
				$price .= $this->get_price_html_from_to_j( $display_regular_price, __( 'Free!', 'woocommerce' ) );
    	...
    	public function get_price_html_from_to_j( $from, $to ) {
		$price = '<li>  <span id="productPrice_del"><del>' . ( ( is_numeric( $from ) ) ? wc_price( $from ) : $from ) . '</del></span>&nbsp;<span id="productPrice"> ' . ( ( is_numeric( $to ) ) ? wc_price( $to ) : $to ) . ' </span> </li>';
	 ...
	 * @return string
	 */
	public function get_image( $size = 'shop_catalog', $attr = array() ) {
		if ( has_post_thumbnail( $this->id ) ) {
